...
- `args` 中调用 `run mysql_mcp_server`，实质上就是运行包内 `main()`
- 数据库配置通过 `env` 字段传入
...
- 使用 `uvx mysql_mcp_server` 启动 MCP Server
- 数据库环境变量在 `env` 中配置
...
- Smithery 会自动调用 `uvx mysql_mcp_server`，并把这些配置注入环境变量

---

## MCP 能力说明（Resources & Tools）

核心逻辑位于 `src/mysql_mcp_server/server.py` 中：
